...
class ItemEntries {
    ...
    private int sizeForPosition(int position) {
        ...
        // item entries.
        if (!mRestoringItem && mAdapterSize != 0 && len > mAdapterSize) {
            len = mAdapterSize;
        }
    ...
    private void ensureSize(int position) {
        if (mItemEntries == null) {
            ...
            int sizeForPosition = Math.max(oldItemEntries.length, sizeForPosition(position));
            mItemEntries = new ItemEntry[sizeForPosition];
